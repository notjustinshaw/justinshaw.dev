        ...
        <ol className="flex flex-col gap-6">
          {posts.filter((post) => post.properties.Category.select.name.toLowerCase().startsWith(query.toLowerCase())
            ...
            return (
              <Link
                key={post.id}
                href={post.properties.Category.select.name === "Youtube" ? post.properties.URL.url : `/${post.properties.Slug.url}`}
                onClick={() => track('click_article', {slug: `/${post.properties.Slug.url}`})}
              >
                <li className="flex flex-row justify-between hover:cursor-pointer">
                  <div className="flex flex-col">
                    ...
                    <p className="text-md text-black/50">
                      ...
                      <span className={`py-0.5 px-1.5 mx-2 text-sm text-black/40 rounded bg-${post.properties.Category.select.color}-100`}>
                        <Link href={search} onMouseEnter={() => track('click_tag', {query: post.properties.Category.select.name})}>
                  ...
                  <div className="flex flex-col justify-end">
                    <span className="text-black/60 hover:text-black/90">{post.properties.Category.select.name === "Youtube" ? "Watch video →" : "Read post →"}</span>
                  </div>
                </li>
              </Link>
            );
